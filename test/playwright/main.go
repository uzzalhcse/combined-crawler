...
func crawlURL(context playwright.BrowserContext, url string, wg *sync.WaitGroup, semaphore chan struct{}, proxy Proxy) {
	defer wg.Done()
...
func main() {
	...
	for i, url := range urls {
		...
		go crawlURL(context, url, &wg, semaphore, proxies[i%len(proxies)])
	}
